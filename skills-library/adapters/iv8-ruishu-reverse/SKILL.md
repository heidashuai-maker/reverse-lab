...
本 skill 复用当前项目内的通用 iv8 规则和 Camoufox 证据流。执行任务时先读取这些文件：

- `../iv8-web-reverse/references/iv8-runtime-rules.md`
- `../iv8-web-reverse/references/camoufox-bridge.md`
- 项目根目录 `skills-library/shared/camoufox-evidence-flow.md`

如果目标已经命中项目 `cases/` 或 `targets/<site>/` 中的可复用瑞数材料，优先读取对应 notes、fixtures 和 verification 证据；不要依赖用户机器上的外部绝对路径。
...
2. 读取 `iv8-web-reverse` 的 `iv8-runtime-rules.md`，使用 `WORK_DIR = Path.cwd()` 和 `CACHE_DIR = WORK_DIR / "js_reverse_cache"` 规则。
3. 用 Camoufox 采集首跳、cookie/storage、关键请求、环境基准和必要的运行时 trace；把证据写入 `targets/<site>/samples/`。
...
- `utils/logger.py` 路径。
- 使用了哪个本地 target/case 作为最近参考；如果没有，明确说明是从零采集。
- 哪些顶部常量控制分页或请求输入。
