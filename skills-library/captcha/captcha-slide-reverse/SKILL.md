...
description: >-
  验证码系列逆向 skill，统一处理极验 GT4 滑块协议、腾讯滑块 TDC/TCaptcha、网易易盾滑块、数美滑块、云片滑块、360 天御滑块这六类任务。适合用户提供 /load、/verify、/api/v3/get、/api/v3/check、/ca/v1/register、/ca/v2/fverify、/v1/jsonp/captcha/get、/v1/jsonp/captcha/verify、360 /api/v3/auth、360 /api/v3/check、cap_union_prehandle、cap_union_new_verify、gcaptcha4.js、code.js、slide.js、TDC.getData(true)、NECaptcha、sm.js、yp.js、rsa_1.js、cb/data/token/trace/rid/i/k/report 样本，并要求还原验证码请求链、动态参数、滑动距离、轨迹或本地 Python/Node 脚本。不要用于通用图片识别、泛化找 sign 入口、只要 DevTools hook、纯 UI 样式问题、或非验证码 WAF/cookie 挑战。
argument-hint: "[目标目录或样本] [平台: gt4/tencent/yidun/shumei/yunpian/tianyu360] [输出: python/node/protocol]"
compatibility: "GT4 通常需要 Python + pycryptodome；腾讯/易盾/数美/云片/360天御常需要 Node.js 或 execjs；图片阶段常用 ddddocr/PIL/OpenCV。"
---
...
1. 极验 GT4：`captcha_id`、`lot_number`、`pow_detail`、`payload`、`process_token`、`/gcaptcha4.geetest.com/load`、`/verify`、`w`、`pow_msg`、`pow_sign`。
2. 腾讯滑块：`TDC`、`getData(true)`、`TCaptcha`、`slide.js`、`drag_ele.html`、`__TENCENT_CHAOS_VM`、`cap_union_prehandle`、`cap_union_new_verify`、`collect`、`eks`。
3. 网易易盾：`NECaptcha`、`c.dun.163.com/api/v3/get`、`/api/v3/check`、`cb`、`data`、`token`、`front/bg`、`yidun`、`dun.163.com/trial/jigsaw`。
4. 数美滑块：`captcha1.fengkongcloud.cn/ca/v1/register`、`/ca/v2/fverify`、`captchaUuid`、`rid`、`fg/bg`、`sm.js`、`get_params.js`、`ww/bb/vj/hq/wi/gq/vs`。
5. 云片滑块：`captcha.yunpian.com/v1/jsonp/captcha/get`、`/captcha/verify`、`captchaId`、`ypjsonp`、`yp_riddler_id`、`yp.js`、`i/k/cb`、`distanceX`、`points`。
6. 360 天御：`captcha.jiagu.360.cn/api/v3/auth`、`/api/v3/check`、`360CaptchaSDK`、`dc1db94ea7b3843c`、`captchaId`、`k`、`report`、`tracking`、`decryptImage`、`rsa_1.js`。
7. 用户要把滑块验证码链路做成本地 Python/Node 脚本，包含图片缺口、轨迹、加密参数和 verify/check 请求。
...
1. 先确认平台：GT4、腾讯 TDC、网易易盾、数美、云片、360 天御不要混用字段和算法。
...
5. 最终验证以服务端响应为准：GT4 看 `result: success`，腾讯看 `cap_union_new_verify`，易盾看 `/api/v3/check` JSONP 内结果，数美看 `/ca/v2/fverify` 是否 `PASS`，云片看 `/captcha/verify` JSONP 内是否返回二次 `token`，360 天御看 `/api/v3/check` 的 `error/data.result`。
...
1. 明确平台和请求链，不混用 GT4、腾讯、易盾、数美、云片、360 天御字段。
2. 动态参数能重新生成，不依赖固定样本值。
3. 图片、轨迹、加密参数和 verify/check 属于同一轮 token/session。
4. 提供可运行脚本或明确指出缺失的动态材料。
5. 失败时打印关键中间值：GT4 的 `wPayload`，腾讯的 `collect/ans/pow_cfg`，易盾的 `token/slide/data/trace length`，数美的 `captchaUuid/rid/x/track length/times/params`，云片的 `captchaId/token/x/distanceX/points length/i/k/cb`，360 天御的 `captchaId/token/k/key/perm_table/raw_x/length/report length`。
