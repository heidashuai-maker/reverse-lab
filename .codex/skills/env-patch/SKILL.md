...
不要使用本 skill 当：
- 还不知道参数、header、cookie 或签名入口在哪，先用 `find-crypto-entry`。
- 只是要 DevTools Console / Snippets 里的观察 hook，先用 `browser-hook-snippets`。
- 目标是还原整份混淆文件的源码结构，先用 `ast-deobfuscate`。
- 只是普通 `412` / `403` / challenge，且没有本地 JS 入口或补环境目标；如果有瑞数证据且目标是固定项目结构、首跳材料缓存、最小 Node runner 或 proxy 观察，交给 `rs-reverse`；如果需要确认防护脚本、入口或调用链，交给 `find-crypto-entry`。
...
浏览器环境真值采集优先使用 Camoufox MCP。Camoufox 作为 Firefox/Gecko 指纹基准工具层，用于 `compare_env`、分批 `evaluate_js`、首屏 pre-inject hook、Cookie / storage / network 证据采集和服务端验收。目标明确走 Chrome/Chromium 指纹分支时，可使用 CloakBrowser MCP 采集 Chrome 基准；CloakBrowser MCP 当前公开路线是 Playwright MCP 兼容工具面加 CloakBrowser Chromium binary。工具层细节见 `references/browser-fingerprint-tool-layer.md`，跨 skill 证据格式见项目根目录的 `skills-library/shared/camoufox-evidence-flow.md`。
...
│     → 需要真实浏览器参数 → 用 Camoufox MCP 采集     │
├─────────────────────────────────────────────────┤
│  5. 循环 3-4 直到成功或稳定                          │
├─────────────────────────────────────────────────┤
│  6. 沙箱指纹比对                                    │
│     → 浏览器基准 vs 本地沙箱同探针 diff              │
│     → 按致命/高危/中危分级写最小补丁                 │
├─────────────────────────────────────────────────┤
│  7. 功能验证 + 服务端验收                            │
│     → 对齐浏览器样本，真实请求连续验收                 │
...
**操作方法**：
1. 用 Camoufox MCP 打开页面并开启网络采集，确认 SDK URL、胶水层脚本和触发请求。
2. 用 `scripts` / `search_code` 搜索 `init`、`setup`、`config`、`beforeSend`、`XMLHttpRequest`、`fetch` 等关键词，定位初始化调用点。
3. 若初始化函数名稳定，用 `hook_function` 或 pre-inject 脚本在刷新前记录入参；若只需读取页面态，用 `evaluate_js` 读取 SDK 全局对象、webpack cache 或配置槽位。
4. 将 init 参数记录到 `targets/<site>/notes.md` 或当前任务的 `js_reverse_cache/` 证据文件中。
...
#### 4b. 用 Camoufox MCP 采集真实参数

当需要真实浏览器指纹值时（如特定网站的 navigator、document、cookie、storage、canvas/WebGL 摘要等），优先使用 Camoufox MCP；目标明确是 Chrome/Chromium 指纹分支时，再用 CloakBrowser MCP 采集 Chrome 基准。

最小采集顺序：

1. 用 `launch_browser` / `navigate` 打开目标页面，必要时先 `network_capture` 再导航。
2. 首屏挑战、WAF 二跳或 SDK 加载即保存原型引用时，用 `navigate(pre_inject_hooks=[...])` 或先 `inject_hook_preset("xhr"|"fetch"|"cookie"|"runtime_probe")` 再刷新。
3. 用 `compare_env` 做粗粒度浏览器/本地差异，用 `evaluate_js` 分批采集 navigator、screen/window、document/performance、canvas/WebGL/Audio 等细项。
4. 用 `cookies`、`get_storage` 保存浏览器状态；Cookie 来源不要依赖单个自动归因工具，组合 `network_capture`、`list_network_requests`、`get_network_request`、`inject_hook_preset("cookie")` 和最终 `cookies` 证据判断 JS 写入还是 HTTP `Set-Cookie`。
5. 将采集值写入 `targets/<site>/samples/` 或当前任务的 `js_reverse_cache/`，再把最小必要值沉淀为 `js_reverse_cache/env/ai-generated/` 补丁。

如果当前会话没有 Camoufox/CloakBrowser 工具，可把 `$SKILL_DIR/scripts/collect-browser-env.js` 作为 DevTools Snippet 执行。采集结果含 cookie、storage 和指纹摘要，默认只保存到当前任务的 `js_reverse_cache/`，不要写入公开 case。
...
当脚本已经能加载，但签名、cookie、header 或服务端响应仍与浏览器不一致时，必须进入 `references/sandbox-fingerprint-validation.md`。

如果目标进入 JSVMP / 安全 SDK 黑箱、XHR/fetch 拦截器“喂入-截出”、jsdom/vm/sdenv 环境伪装，或出现 `HTTP 200 + 空 body`、签名长度正确但服务端拒绝等强环境指纹信号，同时读取 `references/jsdom-env-emulation-playbook.md`。该文件包含路径 B 六步法、分批 `evaluate_js` 采集、`win.eval()` 同探针验证、native `toString` / `Symbol.toStringTag` / `navigator.plugins` / DOM layout 等补丁红线。

最小流程：

1. 先确认入口和初始化链路：导出函数 / XHR 拦截 / fetch 拦截 / `init` / `cacheOpts` / path 白名单。
2. 用 Camoufox MCP 采集 Firefox 浏览器真值；目标明确是 Chrome 分支时，用 CloakBrowser MCP 采集 Chromium 真值。
3. 在本地 jsdom/vm 的目标上下文中运行同一套探针，生成 `browser-env-*.json`、`local-env-baseline.json`、`env-diff.json`。
4. 按致命 / 高危 / 中危分级补最小因果单元，不要批量补全。
5. 保持 UA 家族自洽：Firefox 基准不混入 Chrome-only API，Chrome 基准不混入 Firefox-only API。
6. 在沙箱内部验证 native 外形、plugins、webdriver、document tag、DOM layout 等关键点。

典型强信号：

- `Function.prototype.toString` 暴露 jsdom 或 Node 实现。
- `navigator.plugins.length`、`document.hasFocus()`、DOM layout 与浏览器明显不一致。
- `HTTP 200 + 空 body`、字段少、业务错误码，但浏览器同请求正常。
- 浏览器生成的签名能被 Node/Python 请求接受，本地沙箱生成的签名不被接受。
...
### 步骤 8：封装 + HTTP 中间层 + 请求验证

步骤 7 确认签名值能生成后，再封装为可调用的 `js_reverse_cache/env/sign.js` 或 HTTP 中间层，并用实际请求验证签名是否被服务器接受。
...
2. **`success: true` ≠ 功能可用**：env-diagnose.js 只验证脚本能加载，不验证签名/加密等功能是否正常。必须在 `js_reverse_cache/env/run.js` 中做功能验证（步骤 7）。
...
6. **浏览器基准不能混用**：Camoufox 是 Firefox 指纹基准，CloakBrowser 是 Chrome/Chromium 指纹基准；补环境时必须和目标 UA 分支一致。
...
1. **`success: true` 就收工** — env-diagnose.js 的 success 只代表脚本加载没报错。签名长度不对、格式异常、服务端拒绝，都可能是补环境不完整。必须进入步骤 6 做指纹比对，步骤 7 做功能验证，步骤 8 做真实请求验证。
...
9. **把浏览器自动化当默认方案** — 只有纯协议、Node/vm/jsdom、WASM helper 和 TLS 指纹模拟都无法稳定复现时，才允许浏览器自动化兜底；默认 headless，失败再 headed，并记录不可移植原因。
...
- `references/browser-fingerprint-tool-layer.md`：Camoufox / CloakBrowser / DevTools 的工具层定位和基准选择。
- 项目根目录 `skills-library/shared/camoufox-evidence-flow.md`：Camoufox 采样、环境比对、Cookie 归因和补环境验收证据格式。
- `references/sandbox-fingerprint-validation.md`：沙箱可运行后的环境指纹 diff、补丁分级、UA 自洽和服务端验收闭环。
- `references/jsdom-env-emulation-playbook.md`：JSVMP / SDK 黑箱、jsdom/vm/sdenv 环境伪装、喂入-截出和 native 外形补丁专项。
