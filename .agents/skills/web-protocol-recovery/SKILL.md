...
本 skill 不是默认浏览器自动化 skill。本 skill 是协议恢复 skill；浏览器自动化只允许作为有证据的最后兜底。

默认姿态：

1. 每个新目标以 `chrome-devtools` + `js-reverse` 双工具起步做证据收集
2. 找到真实请求
3. 识别真正的动态状态
4. 离线重建这些状态
5. 优先交付脱离浏览器的 Python 采集器，只在必要时保留极小本地 JS/WASM 参数 helper；协议路径确实无法闭环时，才交付标注清楚的浏览器自动化兜底
...
- 只定位 sign/token/header 的脚本 URL、函数名、调用链 → `find-crypto-entry`
- 已有 JS 入口、要在 Node.js 沙箱里补环境跑通 → `env-patch`
- 整文件 AST 解混淆、控制流还原 → `ast-deobfuscate`
- 明确要紧凑 Python + iv8 执行浏览器 JS 并翻页请求 → `iv8-web-reverse`
- 核心目标仅为单个 WAF/反爬 cookie（如 w_tsfp），不涉及完整协议链路 → `waf-cookie-pure-first`
...
- 最终交付优先纯协议：原始 HTTP + 本地签名/解码/bootstrap helper
- 每个新目标必须先经浏览器工具层 + `js-reverse` 双工具分析再写最终采集器
- Camoufox MCP 是默认浏览器工具层，用于 Firefox 指纹基准、环境 diff、hook、Cookie 归因和服务端验收
- CloakBrowser MCP 可作为 Chrome/Chromium 指纹基准和自动化兜底工具层；使用前应确认 MCP 可用
- 正常最终交付在浏览器外运行：Python 采集器 + 仅在 Python 移植不划算时保留极小本地 JS/WASM helper
- JS helper 必须能在本地运行，不依赖 `document`/`window`/人工点击/浏览器 profile
- 浏览器工具只做侦察和证据收集，不作为最终采集器的隐藏依赖；若作为兜底交付，必须显式标注为 browser-auto
- 浏览器自动化只能在纯 Python、纯 Node、vm/jsdom/sdenv、WASM helper、TLS/HTTP 指纹模拟均无法闭环后使用；默认 headless，失败再 headed
...
每个 fresh target 开始前，先完成三项检查：

1. **环境与工具检查**：本地可用时运行 `scripts/check_reverse_env.py`；确认 `chrome-devtools` 和 `js-reverse` 是否可用；若有工具不可用，先报告阻塞再继续
2. **缓存目录检查**：确认当前工作目录下存在 `js_reverse_cache/`，不存在则创建；后续所有下载的 JS/WASM/字体/HTML/抓包样本均写入该目录
3. **Target artifact 检查**：先读 `references/target-artifact-contract.md`；确认长期证据写入 `targets/<source-name>/`，并明确本轮要更新哪些 `samples/*.jsonl`、`scripts/env/*` 或 `output/*`
4. **请求链证据检查**：先读 `references/request-chain-evidence.md`；确认目标请求、writer/builder/entry/source、上游依赖和样本状态是否已经有证据闭环
5. **目标家族分类**：先读 `references/startup-triage-playbook.md`，将目标分类为 `signer-gated` / `verifier-gated` / `decode-gated` / `session-gated`
6. **交付意图声明**：明确最终交付形态——纯 Python / Python+极小 JS helper / Python+极小 WASM helper / Python+本地 bootstrap executor / browser-auto 兜底；若进入 browser-auto，必须先说明前置路径为何不能闭环
...
每个新目标必须先启动 gate，再做轻量双工具分析：

1. `scripts/check_reverse_env.py`（本地执行可用时）+ `references/startup-triage-playbook.md` 分类
2. Camoufox MCP 轻量侦察：页面状态、跳转链、首轮网络视图、Firefox 指纹基准、Cookie 归因；不可用时用当前可用浏览器工具替代并记录缺口
3. `js-reverse` 轻量侦察：initiator 栈、源码搜索、wrapper 追踪、突变假设
4. 目标明确走 Chrome/Chromium 指纹分支时，补 CloakBrowser MCP 采集 Chrome 基准

不要跳过任一工具；若被封阻，明确报告。

优先级顺序：startup triage → Camoufox/browser 侦察 → js-reverse 溯源 → request diff → helper 固定输入验证 → 离线执行 → 本地协议回放 → 必要时 browser-auto 兜底。
...
5. TLS / HTTP 指纹模拟（浏览器值可用但本地 HTTP 客户端失败）
6. 浏览器自动化兜底（只有前面路径被证据排除后，优先 headless，其次 headed）

不要选择：未做前置验证就浏览器驱动回放、"我的浏览器 profile 里能跑"、把 browser-auto 伪装成纯协议交付。
...
- `scripts/scaffold_reverse_project.py`：生成 protocol-first 的项目骨架
...
| 场景 | Reference |
|------|-----------|
| target 证据文件、阶段产物、追加/覆盖规则 | `references/target-artifact-contract.md` |
| 目标请求、字段来源、writer/builder/entry/source 未闭环 | `references/request-chain-evidence.md` |
| 阶段切换、子 skill 交接、会话续做 | `references/stage-handoff-protocol.md` |
...
| Python / Node / vm / WASM / TLS / browser-auto 路径选择 | `references/reproduction-path-matrix.md` |
...
| 浏览器与本地执行 first divergence | `references/runtime-divergence-diagnosis.md` |
...
| WASM / Worker / Webpack runtime 桥接层 | `references/bridge-contracts.md` |
...
Target Artifact Contract → `references/target-artifact-contract.md`
Request Chain Evidence → `references/request-chain-evidence.md`
Stage Handoff Protocol → `references/stage-handoff-protocol.md`
