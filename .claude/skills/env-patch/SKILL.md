...
浏览器环境真值采集优先使用 Camoufox MCP。Camoufox 作为 Firefox/Gecko 指纹基准工具层，用于 `compare_env`、分批 `evaluate_js`、首屏 pre-inject hook、Cookie 归因和服务端验收。目标明确走 Chrome/Chromium 指纹分支时，可使用 CloakBrowser MCP 采集 Chrome 基准；CloakBrowser MCP 当前公开路线是 Playwright MCP 兼容工具面加 CloakBrowser Chromium binary。工具层细节见 `references/browser-fingerprint-tool-layer.md`。
...
当需要真实浏览器指纹值时（如特定网站的 navigator.userAgent、document.cookie 等），优先使用已配置的 `js-reverse` MCP 浏览器操作工具；如果当前会话没有暴露该 MCP，再使用可用的浏览器调试工具等价采集：

1. 用 `js-reverse` 的 `new_page` 能力打开目标网站。
2. 用 `js-reverse` 的 `evaluate_script` 能力采集需要的属性值；如果当前客户端暴露的是其它浏览器 MCP，则使用等价的页面执行脚本能力：
   ```javascript
   () => ({
