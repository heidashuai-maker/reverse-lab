...
<!-- 共享协议：此文件与 ast-deobfuscate / browser-hook-snippets / find-crypto-entry / iv8-web-reverse / rs-reverse / web-protocol-recovery 中的副本须保持同步，修改时一并更新。 -->

这份共享参考用于串联 `rs-reverse`、`find-crypto-entry`、`browser-hook-snippets`、`env-patch`、`ast-deobfuscate`、`iv8-web-reverse` 和 `web-protocol-recovery`。它不是独立 skill，而是前端 JS 逆向任务的阶段协议。
...
上游: `find-crypto-entry` / `rs-reverse`
...
上游: `browser-hook-snippets` / `find-crypto-entry`
下游: `env-patch`
...
如果当前任务更偏"请求链定位和会话续接"，至少维护一个轻量交接文档，例如 `reverse-records/请求链路.md` 或项目里的等价文件，用来持续记录：目标请求、目标字段、样本编号、证据编号、来源链、去向链、未闭环项。
...
对于以请求链为主的任务，交接卡应指向 `reverse-records/请求链路.md` 或等价工件；对于运行时差异任务，交接卡应指向 `runtime-evidence`、`first divergence` 记录或验证夹具。
...
2. 要找参数入口、脚本位置和调用链：用 `find-crypto-entry`。
