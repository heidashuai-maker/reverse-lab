...
如果用户真正卡住的是参数入口、header 来源或调用链，转给 `find-crypto-entry`。
...
1. 目标项目中的 `project/scripts/deobfuscate_target.js` 或按步骤拆分的 `project/scripts/stepN_*.js`
...
原则：`project/scripts/deobfuscate_target.js`、`project/scripts/stepN_*.js`、`project/decode-action-workspace/` 都是执行 skill 时写入目标项目的产物；skill 自带资源只有 `scripts/decode_action_scaffold.js` 和 `scripts/template-project/`。每一步读取上一步输出，生成新文件，不覆盖原始输入。
...
1. 目标项目中的 `project/scripts/instrument_target.js`
